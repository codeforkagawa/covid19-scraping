...
def generateSummary(updated_at):
    ...
    for i, row in enumerate(reader):
        if i == 0:
            continue
        print(row)
        inspectionTemplate["data"]["県内"].append(int(row["PCR検査件数(環境保健研究センター)"].strip() or 0)+int(row["PCR検査件数(その他)"].strip() or 0)+int(row["抗原検出用キット実施件数(医療機関)"].strip() or 0)+int(row["抗原検出用キット実施件数(保健所)"].strip() or 0))
        ...
        total_inspections += int(row["PCR検査件数(環境保健研究センター)"].strip() or 0)+int(row["PCR検査件数(その他)"].strip() or 0)+int(row["抗原検出用キット実施件数(医療機関)"].strip() or 0)+int(row["抗原検出用キット実施件数(保健所)"].strip() or 0)
    # 検査陽性者の状況
